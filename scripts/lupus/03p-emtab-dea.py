...
"""
Best Practices Plotting Script for DESeq2 Pseudo-bulk Results (EMTAB Lupus)
==========================================================================

This script visualizes differential expression (DE) results from the pseudo-bulk DE pipeline
for the EMTAB lupus dataset. It generates:
...
Inputs:
    ...
    - Complement gene list: resources/complement-genes.txt

Outputs:
    - Plots and summary tables in plots/lupus-emtab/dea/
...
def setup_logging():
    logger = logging.getLogger("emtab_dea_plotting")
...
def plot_volcano(celltype_to_df, complement_genes, plotdir, logger):
    ...
    for celltype, df in celltype_to_df.items():
        ...
        plt.xlabel("log2 Fold Change (Disease A vs Disease B)")
...
def plot_heatmap(celltype_to_df, complement_genes, celltypes, plotdir, logger):
    ...
    plt.title("Complement Genes: log2 Fold Change per Cell Type")
    ...
    plt.suptitle("Clustermap: Complement Genes log2 Fold Change per Cell Type")
