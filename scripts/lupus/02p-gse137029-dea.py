...
"""
02p-gse137029-dea.py

Best Practices Plotting Script for DESeq2 Pseudo-bulk Results (GSE137029, Lupus PBMC)
=====================================================================================
- Visualizes differential expression (DE) results from the pseudo-bulk DE pipeline
  for the GSE137029 lupus PBMC dataset.
- Generates:
    ...
    - Summary table of top DE complement genes per cell type
...
Author: Expert Engineer (2024, refactored for modularity, logging, versioning)
"""
...
try:
    ...
    from lupus.utils.common import log_environment, save_version_info, setup_logging

# --- Configurable Paths ---
OUTDIR = (
    "/Users/aumchampaneri/complement-receptor-blockade/outputs/lupus-gse137029/dea/"
)
PLOTDIR = "/Users/aumchampaneri/complement-receptor-blockade/plots/lupus-gse137029/dea/"
GENE_LIST_PATH = (
...
def plot_volcano(celltype_to_df, complement_genes, plotdir, logger):
    ...
    for celltype, df in celltype_to_df.items():
        ...
        plt.xlabel("log2 Fold Change (Lupus vs Control)")
...
def plot_heatmap(celltype_to_df, complement_genes, celltypes, plotdir, logger):
    ...
    for celltype, df in celltype_to_df.items():
        vals = df["log2FoldChange"].reindex(complement_genes)
        log_vals = np.sign(vals) * np.log2(1 + np.abs(vals))
        log2fc_matrix[celltype] = log_vals
    plt.figure(
    ...
    sns.heatmap(
        ...
        linewidths=0.5,
        linecolor="gray",
        cbar_kws={"label": "log2(1 + |log2FC|) × sign"},
        vmin=-2,
        vmax=2,
    )
    plt.title(
        "Complement Genes: log2(1 + |log2FC|) × sign (Lupus vs Control) per Cell Type"
    )
    ...
    plt.suptitle(
        "Clustermap: Complement Genes log2(1 + |log2FC|) × sign (Lupus vs Control)"
    )
...
def plot_dotplot(celltype_to_df, complement_genes, celltypes, plotdir, logger):
            ...
            dotplot_data.append(
                {
                    ...
                    "celltype": str(celltype) if celltype is not None else "",
                    "log2fc": log2fc,
                    "padj": padj,
                    "sig": sig,
                }
    ...
    # Drop rows with missing, empty, or non-string gene/celltype
    dotplot_df = dotplot_df[
        dotplot_df["gene"].apply(lambda x: isinstance(x, str) and x.strip() != "")
        & dotplot_df["celltype"].apply(lambda x: isinstance(x, str) and x.strip() != "")
        & dotplot_df["gene"].isin(complement_genes)
        & dotplot_df["celltype"].isin(celltypes)
    ]
    dotplot_df["log2fc_capped"] = dotplot_df["log2fc"].clip(lower=-3, upper=3)

    # Map categories to integer codes for robust plotting
    gene_to_y = {g: i for i, g in enumerate(complement_genes)}
    celltype_to_x = {ct: i for i, ct in enumerate(celltypes)}
    dotplot_df = dotplot_df[
        dotplot_df["gene"].isin(gene_to_y) & dotplot_df["celltype"].isin(celltype_to_x)
    ]
    x = dotplot_df["celltype"].map(celltype_to_x)
    y = dotplot_df["gene"].map(gene_to_y)

    # Log-scale for dotplot color
    dotplot_df["log2fc_log"] = np.sign(dotplot_df["log2fc"]) * np.log2(
        1 + np.abs(dotplot_df["log2fc"])
    )
    dotplot_df["log2fc_log_capped"] = dotplot_df["log2fc_log"].clip(lower=-2, upper=2)
    # Debug: print unique values
    logger.info(f"Dotplot genes: {dotplot_df['gene'].unique()}")
    logger.info(f"Dotplot celltypes: {dotplot_df['celltype'].unique()}")
    plt.figure(
        figsize=(max(12, len(celltypes) * 0.6), max(8, len(complement_genes) * 0.25))
    )
    sc = plt.scatter(
        ...
        c=dotplot_df["log2fc_log_capped"],
        cmap="vlag",
        vmin=-2,
        vmax=2,
        edgecolor="k",
    ...
    plt.colorbar(sc, label="log2(1 + |log2FC|) × sign")
    plt.title(
        "Seurat-style Dotplot: Complement Genes × Cell Types\nDot size = significant (padj<0.05), color = log2(1 + |log2FC|) × sign"
    )
    plt.xlabel("Cell Type")
    plt.ylabel("Complement Gene")
    plt.xticks(ticks=np.arange(len(celltypes)), labels=celltypes, rotation=90)
    plt.yticks(ticks=np.arange(len(complement_genes)), labels=complement_genes)
    plt.tight_layout()
...
def main():
    ...
    # Debug: Check complement gene presence in DESeq2 results for each cell type
    for celltype, df in celltype_to_df.items():
        present = [g for g in complement_genes if g in df.index]
        print(f"{celltype}: {len(present)} complement genes found in DESeq2 results")
        if len(present) < 5:
            print("Example present:", present[:5])
            print(
                "Example missing:",
                [g for g in complement_genes if g not in df.index][:5],
            )
