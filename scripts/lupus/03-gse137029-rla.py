#!/usr/bin/env python3
"""
Multi-Resource LIANA Receptor-Ligand Analysis & Consensus Aggregation for Lupus GSE137029
=========================================================================================
...
INPUT:
    - AnnData: /Users/aumchampaneri/complement-receptor-blockade/data/lupus-gse137029/gse137029_qc_harmony.h5ad
...
OUTPUT:
    ...
    - Consensus table:
        /Users/aumchampaneri/complement-receptor-blockade/outputs/lupus-gse137029/rla/consensus_complement_interactions.csv

ENVIRONMENT:
    - Python >=3.8
    - liana, scanpy, pandas, mygene

AUTHOR: Automated Pipeline (multi-resource, lupus GSE137029)
DATE: 2024-06
"""
...
# ------------------- Configurable Paths -------------------
DATA_PATH = "/Users/aumchampaneri/complement-receptor-blockade/data/lupus-gse137029/gse137029_qc_harmony_genesymbols.h5ad"
GENE_LIST_PATH = (
...
celltype_col = None
for candidate in ["cell_type", "celltype", "celltypist_label", "CellType"]:
    if candidate in adata.obs.columns:
        celltype_col = candidate
        break
if celltype_col is None:
    raise ValueError(
        f"Could not find a cell type annotation column in AnnData.obs. "
        f"Tried: cell_type, celltype, celltypist_label, CellType. "
...
# Check overlap
overlap = set(g.upper() for g in adata.var_names) & set(
    g.upper() for g in complement_genes
)
print(f"Number of complement genes present in AnnData: {len(overlap)}")

if len(overlap) > 0:
    print(
        "Gene symbols in AnnData match complement gene list. Using gene symbols directly."
    )
    ...
    try:
        ...
        raise ImportError("Please install mygene: pip install mygene")
...
for iid, group in grouped:
    row = {
        ...
        "resources": ",".join(sorted(group["resource"].unique())),
        "mean_lrscore": group["lrscore"].mean() if "lrscore" in group else None,
        "max_lrscore": group["lrscore"].max() if "lrscore" in group else None,
        "min_lrscore": group["lrscore"].min() if "lrscore" in group else None,
    }
    agg_rows.append(row)

consensus_df = pd.DataFrame(agg_rows)
consensus_df = consensus_df.sort_values(
    ["n_resources", "mean_lrscore"], ascending=[False, False]
)

consensus_csv = os.path.join(OUTDIR, "consensus_complement_interactions.csv")
consensus_df.to_csv(consensus_csv, index=False)
print(f"Saved consensus table to {consensus_csv}")
